...
// #define GPIOC_ASCR        (*(volatile uint32_t *)(GPIOC_BASE + 0x2C)) // ASCR
// offset is 0x2C on L4
...
// oversampling #define ADC1_SMPR1        (*(volatile uint32_t *)(ADC1_BASE +
// 0x14)) #define ADC1_SMPR2        (*(volatile uint32_t *)(ADC1_BASE + 0x18))
...
// #define ADC_CFGR2_OVSE       (1U << ADC_CFGR2_OVSE_Pos)    // oversampler
// enable #define ADC_CFGR2_OVSR_Pos   2                             // 3 bits
...
//     while (us--) {
//         for (volatile uint32_t i = 0; i < 20; ++i) { __asm__ volatile("nop");
//         }
//     }
// }
...
//     ADC1_CFGR2 = (ADC1_CFGR2 & ~(ADC_CFGR2_OVSR_MASK | ADC_CFGR2_OVSS_MASK |
//     ADC_CFGR2_ROVSM))
//                | ADC_CFGR2_OVSE
//                | ADC_CFGR2_OVSR_256
//                | ADC_CFGR2_OVSS_4
...
//         if (t == 199999) {
//             printf("[ADC] ADRDY timeout: ISR=0x%08lx, CR=0x%08lx,
//             CCR=0x%08lx\r\n",
...
//         }
//     }
//     // With oversampling (256x, shift 4), DR now holds ~16-bit range
//     (0..65535 nominal) return (uint16_t)ADC1_DR;
// }

// int main(void) {
//     printf("Starting ADC readings on PC5 (A0 -> ADC1_IN14) with 256x
//     oversampling...\r\n");
...
//         float v = (float)raw16 * (3.3f / 65535.0f);          // assumes VDDA
//         ≈ 3.3 V printf("ADC16 Raw: %u, Voltage: %.4f V\r\n", raw16, v);
//         ThisThread::sleep_for(500ms);
//     }
// }
...
// #include "mbed.h" // For ThisThread::sleep_for and printf
...
//     GPIOC_MODER |= (0x3U << (PIN_C3*2)) | (0x3U << (PIN_C4*2)) | (0x3U <<
//     (PIN_C5*2)); GPIOC_PUPDR &= ~((0x3U << (PIN_C3*2)) | (0x3U << (PIN_C4*2))
//     | (0x3U << (PIN_C5*2))); GPIOC_ASCR  |= (1U << PIN_C3) | (1U << PIN_C4) |
//     (1U << PIN_C5);
// }

// static void adc_set_sample(uint32_t ch, uint32_t smp) {
//     if (ch <= 9) ADC1_SMPR1 = (ADC1_SMPR1 & ~(0x7U << (3*ch))) | (smp <<
//     (3*ch)); else         ADC1_SMPR2 = (ADC1_SMPR2 & ~(0x7U << (3*(ch-10))))
//     | (smp << (3*(ch-10)));
// }
...
//     delay_us(30);
//     ADC_CCR = (ADC_CCR & ~(3U << ADC_CCR_CKMODE_Pos)) | (1U <<
//     ADC_CCR_CKMODE_Pos); ADC1_CR |= ADC_CR_ADCAL; while (ADC1_CR &
//     ADC_CR_ADCAL); ADC1_CFGR &= ~ADC_CFGR_ALIGN; adc_set_sample(CH_A0,
//     0b101); adc_set_sample(CH_A1, 0b101); adc_set_sample(CH_A2, 0b101);
//     ADC1_CFGR2 = ADC_CFGR2_OVSE | ADC_CFGR2_ROVSM | ADC_CFGR2_OVSR_256 |
//     ADC_CFGR2_OVSS_4; ADC1_ISR |= ADC_ISR_ADRDY; ADC1_CR |= ADC_CR_ADEN; for
//     (volatile uint32_t t = 0; t < 200000; ++t)
...
//         a1.read() * 3.3f, a2.read() * 3.3f); ThisThread::sleep_for(1s);
//     }
// }
...
/*
    DAC Output Example: May not work
*/

// #include "mbed.h"

// // On STM32 Nucleo boards, DAC output is usually on pin A2 (PA_4)
// AnalogOut dac_output(PA_5); // Replace PA_4 with your board’s DAC-capable pin
...
//     while (true)
//     {
//         // Output a value from 0.0 to 1.0 (0V to Vref, usually 3.3V)
//         dac_output = val; // This gives ~1.65V if Vref is 3.3V
//         printf("DAC output set to %.2f (%.2f V)\r\n", val, val * 3.3f);
...
// AnalogOut dac_out(A4); // PA_4 supports DAC output
...
//         {
//             dac_out.write(val); // val must be between 0.0 and 1.0
